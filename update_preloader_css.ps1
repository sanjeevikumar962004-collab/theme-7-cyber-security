...
foreach ($file in $files) {
    ...
    if (Test-Path $path) {
        ...
        if ($content -notmatch 'Light Mode Preloader Overrides') {
            # Search for @keyframes glitch-text { ... } </style>
            # The (?s) flag allows . to match newline.
            # So (?s)(@keyframes glitch-text \{.*?\r?\n)</style>
            $content = [regex]::Replace($content, '(?s)(@keyframes glitch-text\s*\{.*?\r?\n)</style>', "`$1`n${lightModeCSS}")
            
            [IO.File]::WriteAllText($path, $content)
            Write-Host "Updated CSS in $file"
        }
    }
}
Write-Host "Done updating CSS."
